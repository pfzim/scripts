...
try
{
    $data = & 'C:\Program Files\Veritas\NetBackup\bin\admincmd\bppllist.exe' -json
...
{
    if($p.class_name -match "^FILE")
    {
        foreach($c in $p.clients)
        {
            $clients += $c.hostname
        }
    }
...
<body>
<h1>NetBackup FILE backup status</h1>
<table>
...
foreach($client in $clients)
{
    ...
    {
        if($image.policy_name -match "^FILE")
        {
...
Send-MailMessage -from $smtp_from -to $smtp_to -Encoding UTF8 -subject "NetBackup FILE backup status" -bodyashtml -body $body -smtpServer $smtp_server -Credential $smtp_creds
