...
for($i = 0; $i -lt $data.Count; $i++)
{
    ...
    if($row -match '^CLASS ([^\s]+)')
    {
        $class = $matches[1]
        ("" + $class)
        $skip_class = ($class -notmatch "^FILE")
        $skip_schedule = $true
        if(!$skip_class)
        {
            $policies[$class] = @{}
...
foreach($p_key in $policies.Keys)
{
    ...
    $c_key.name
        $summary[$p_key] = @{}
        $summary[$p_key][$c_key.name] = @{Full = (Get-Date "1970-01-01 00:00:00.000Z"); FullName = "None"; Diff = (Get-Date "1970-01-01 00:00:00.000Z"); DiffName = "None"}
    }
}
...
foreach($log_date in $log_dates)
{
	...
	foreach($j in $json)
	{
		if($j.PolicyName -match "^FILE" -and $j.State -eq 3 -and $j.Status -eq 0 -and $j.Restartable -and ($j.ScheduleType -eq 0 -or $j.ScheduleType -eq 1 -or $j.ScheduleType -eq 3))
		{
			...
			Write-Host -ForegroundColor Gray ("{0}`t{9}`t{1}`t{2}`t{3}`t{4}`t{5}`t{6}`t{7}`t{8}`t{10}`t{11}" -f $j.JobId,$jst.ToString("dd.MM.yyyy HH:mm"),$jet.ToString("dd.MM.yyyy HH:mm"),$j.PolicyName,$j.ScheduleName,$j.ClientName,$j.InstanceDatabaseName,$j.Status,$j.JobSubtypeText, $j.ParentJobId, $j.State, $j.ScheduleType)
			...
			{
				continue
				$summary[$j.PolicyName][$j.ClientName] = @{Full = (Get-Date "1970-01-01 00:00:00.000Z"); FullName = "None"; Diff = (Get-Date "1970-01-01 00:00:00.000Z"); DiffName = "None"}
			}

			if($j.ScheduleType -eq 0 -or $j.ScheduleType -eq 3)
			{
				if($summary[$j.PolicyName][$j.ClientName].Full -le $jst)
				{
				   $summary[$j.PolicyName][$j.ClientName].Full = $jst
				   $summary[$j.PolicyName][$j.ClientName].FullName = $j.ScheduleName
				}
			}
			elseif($j.ScheduleType -eq 1)
			{
				if($summary[$j.PolicyName][$j.ClientName].Diff -le $jst)
				{
				   $summary[$j.PolicyName][$j.ClientName].Diff = $jst
				   $summary[$j.PolicyName][$j.ClientName].DiffName = $j.ScheduleName
				}
			}
...
$title = "NetBackup FILE backup status"
...
$body += @'
<table>
<tr><th>Policy</th><th>Client</th><th>Full</th><th>Schedule</th><th>Diff</th><th>Schedule</th></tr>
'@
...
foreach($p_key in $summary.Keys)
{
    ...
    {
		if($summary[$p_key][$c_key].Diff -le $date7)
		{
			$s_diff = '<td class="error">{0}</td>' -f ($summary[$p_key][$c_key].Diff).ToString("dd.MM.yyyy HH:mm")
		}
		elseif($summary[$p_key][$c_key].Diff -le $date1)
		{
			$s_diff = '<td class="warn">{0}</td>' -f ($summary[$p_key][$c_key].Diff).ToString("dd.MM.yyyy HH:mm")
		}
		else
		{
			$s_diff = '<td class="pass">{0}</td>' -f ($summary[$p_key][$c_key].Diff).ToString("dd.MM.yyyy HH:mm")
		}

		if($summary[$p_key][$c_key].Full -le $date14)
		{
			$s_full = '<td class="error">{0}</td>' -f ($summary[$p_key][$c_key].Full).ToString("dd.MM.yyyy HH:mm")
		}
		elseif($summary[$p_key][$c_key].Full -le $date7)
		{
			$s_full = '<td class="warn">{0}</td>' -f ($summary[$p_key][$c_key].Full).ToString("dd.MM.yyyy HH:mm")
		}
		else
		{
			$s_full = '<td class="pass">{0}</td>' -f ($summary[$p_key][$c_key].Full).ToString("dd.MM.yyyy HH:mm")
		}
				
		$body += "<tr><td>{0}</td><td>{1}</td>{2}<td>{4}</td>{3}<td>{5}</td></tr>`r`n" -f $p_key, $c_key, $s_full, $s_diff, $summary[$p_key][$c_key].FullName, $summary[$p_key][$c_key].DiffName
   }
}
